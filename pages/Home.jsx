...
const Home = ({ data }) => {
  ...
  // Reusable component for small grainy gradient spots
  const GrainySpot = ({ className, color = "rgba(99, 102, 241, 0.12)" }) => (
    <div className={`absolute pointer-events-none blur-[50px] ${className}`}>
      <div 
  ...
  return (
    <div className="flex flex-col">
      ...
      <section className="min-h-screen flex flex-col items-center justify-center relative px-4 overflow-hidden">
        ...
        {/* Strategically Placed Grainy Accents */}
        <GrainySpot className="top-[20%] left-[15%] w-64 h-64 animate-blob" color="rgba(99, 102, 241, 0.12)" />
        <GrainySpot className="bottom-[35%] right-[10%] w-72 h-72 animate-blob animation-delay-2000" color="rgba(139, 92, 246, 0.1)" />
        <GrainySpot className="top-[45%] left-[45%] w-32 h-32 opacity-40" color="rgba(45, 212, 191, 0.08)" />
        ...
        <div className="flex flex-col items-center text-center max-w-4xl z-10">
          ...
          <div className="reveal active delay-100 mb-6">
            <h1 className="text-6xl sm:text-7xl lg:text-9xl font-black text-white tracking-tighter leading-[0.85] mb-4">
              {profile.name}
            </h1>
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-indigo-400 via-violet-400 to-cyan-400 tracking-tight">
              {profile.title}
            </h2>
          </div>

          {/* Tagline */}
          <p className="reveal active delay-200 text-zinc-400 text-lg sm:text-xl mb-12 max-w-lg font-medium leading-relaxed">
            {profile.tagline}
          </p>
          ...
          <div className="reveal active delay-300 flex flex-col sm:flex-row gap-4 mb-16 w-full sm:w-auto">
            <Link
              to="/projects"
              className="inline-flex items-center justify-center gap-2 px-10 py-4 bg-white text-black text-[11px] font-black uppercase tracking-widest rounded-xl hover:bg-indigo-600 hover:text-white transition-all shadow-xl shadow-white/5 active:scale-95"
            >
              View Projects <ArrowRight size={18} />
            </Link>
            <a
              href="#contact"
              className="inline-flex items-center justify-center gap-2 px-10 py-4 border border-zinc-800 bg-zinc-950/50 text-white text-[11px] font-black uppercase tracking-widest rounded-xl hover:bg-zinc-900 transition-all active:scale-95"
            >
          ...
          {/* Social Row */}
          <div className="reveal active delay-400 flex items-center gap-6">
            <a href={profile.github} target="_blank" rel="noreferrer" className="text-zinc-500 hover:text-white transition-all hover:scale-110">
              <Github size={22} />
            </a>
            <a href={profile.linkedin} target="_blank" rel="noreferrer" className="text-zinc-500 hover:text-white transition-all hover:scale-110">
              <Linkedin size={22} />
            </a>
            <a href={`mailto:${profile.email}`} className="text-zinc-500 hover:text-white transition-all hover:scale-110">
              <Mail size={22} />
            </a>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
                <div className="reveal">
                    ...
                    <h2 className="text-4xl lg:text-5xl font-black mb-8 text-white tracking-tighter">Engineering for the <br/><span className="text-zinc-600 font-medium italic underline decoration-indigo-500/20 underline-offset-8">next digital frontier.</span></h2>
